...
export const createApplication = async (req, res) => {
  try {
    ...
    // Check job exists and is still active
    ...
    await createNotification({
      ...
      userId: job.postedBy, // ✅ notify the recruiter
    });
...
export const updateApplication = async (req, res) => {
  try {
    ...
    // Can only update if still in APPLIED status
    ...
    if (existing.status !== "APPLIED") {
      return res.status(400).json({
        success: false,
        message: "Application can no longer be edited once it has been reviewed",
      });
    }
...
export const updateApplicationStatus = async (req, res) => {
  try {
    ...
    // ✅ Populate jobId so we can access job title for notification
    ...
    // ✅ Null check before accessing application fields
    if (!application) {
      return res.status(404).json({
        success: false,
        message: "Application not found",
    ...
    // ✅ Notify the candidate about their status update
    await createNotification({
