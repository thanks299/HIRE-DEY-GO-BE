...
describe("Job Controller Unit", () => {
  test("getJobs should apply pagination/sorting and return shaped response", async () => {
    ...
    const limitSpy = mock.fn(async () => fakeJobs);
    const skipSpy = mock.fn(() => ({ limit: limitSpy }));
    const sortSpy = mock.fn(() => ({ skip: skipSpy }));

    const findMock = mock.method(Job, "find", () => ({ sort: sortSpy }));
